...
func TestAAPF(t *testing.T) {
	tests := []string{
		"www.zdns.cn. 3600 IN AAPF 201::0",
		"www.zdns.cn. 3600 IN AAPF FFEE::2:1",
	}

	for _, t1 := range tests {
		buf := make([]byte, 1024)
		rr, err := NewRR(t1)
		//fmt.Printf("parse NewRR for str:%s get:%v err:%v\n", t1, rr, err)
		off, err := PackRR(rr, buf, 0, nil, false)
		if err != nil {
			t.Errorf("failed to pack AAAA:%v: %s", err, t1)
			continue
			//} else {
			//	fmt.Printf("pack AA len:%d.\n", off)
			//	for i := 0; i < off; i++ {
			//		fmt.Printf("%d:%d\n", i, buf[i])
			//	}
		}
		...
		rr, _, err = UnpackRR(buf[:off], 0)
		if err != nil {
			t.Errorf("failed to unpack AAPF:%v: %s", err, t1)
		}
		t.Log(rr)
		//fmt.Printf("after unpack AAPF rr is %s.\n", rr.String())
	}

	//fmt.Printf("AAPF succceeed.\n")
}

func TestAA(t *testing.T) {
	tests := []string{
		"www.zdns.cn. 3600 IN AA 1.1.1.1",
		"www.zdns.cn. 3600 IN AA 192.168.0.1",
	}
	...
	for _, t1 := range tests {
		...
		off, err := PackRR(rr, buf, 0, nil, false)
		if err != nil {
			t.Errorf("failed to pack AA:%v: %s", err, t1)
			continue
			//} else {
			//	fmt.Printf("pack AA len:%d.\n", off)
			//	for i := 0; i < off; i++ {
		...
		rr, _, err = UnpackRR(buf[:off], 0)
		if err != nil {
			t.Errorf("failed to unpack AA:%v: %s", err, t1)
		}
		t.Log(rr)
		//fmt.Printf("after unpack AA rr is %s.\n", rr.String())
	}

	//fmt.Printf("AA succceeed.\n")
}

func TestTRANSAA(t *testing.T) {
	tests := []string{
		"a.cn. 3600 IN TRANSAA 1 1.1.1.1",
		"a.cn. 3600 IN TRANSAA 1 b.cn.",
	}
	...
	for _, t1 := range tests {
		...
		off, err := PackRR(rr, buf, 0, nil, false)
		if err != nil {
			t.Errorf("failed to pack transaa %v: %s", err, t1)
			continue
		}

		rr, _, err = UnpackRR(buf[:off], 0)
		if err != nil {
			t.Errorf("failed to unpack transaa %v: %s", err, t1)
		}
		t.Log(rr)
	}
}
...
func TestTRANS(t *testing.T) {
	tests := []string{
		"a.cn. 3600 IN TRANS 1 1.1.1.1",
		"a.cn. 3600 IN TRANS 1 b.cn.",
		"a.cn. 3600 IN TRANS 1 201::1",
	}
	...
	for _, t1 := range tests {
		...
		off, err := PackRR(rr, buf, 0, nil, false)
		if err != nil {
			t.Errorf("failed to pack trans %v: %s", err, t1)
			continue
			//} else {
			//	fmt.Printf("pack trans len:%d.\n", off)
			//	for i := 0; i < off; i++ {
			//		fmt.Printf("%d:%d\n", i, buf[i])
			//	}
		}
		...
		rr, _, err = UnpackRR(buf[:off], 0)
		if err != nil {
			t.Errorf("failed to unpack trans %v: %s", err, t1)
		}
		t.Log(rr)
		//fmt.Printf("after unpack trans rr is %s.\n", rr.String())
	}

	//fmt.Printf("trans succceeed.\n")
}

func TestXW(t *testing.T) {
	tests := []string{
		"a.cn. 3600 IN XW 1 1.2.3.4",
		"a.cn. 3600 IN XW 1 b.cn",
	}
	buf := make([]byte, 1024)
	for _, t1 := range tests {
		rr, _ := NewRR(t1)
		off, err := PackRR(rr, buf, 0, nil, false)
		if err != nil {
			t.Errorf("failed to pack XW %v: %s", err, t1)
			continue
			//} else {
			//	fmt.Printf("pack xw len:%d.\n", off)
			//	for i := 0; i < off; i++ {
			//		fmt.Printf("%d:%d\n", i, buf[i])
			//	}
		}
		...
		rr, _, err = UnpackRR(buf[:off], 0)
		if err != nil {
			t.Errorf("failed to unpack XW %v: %s", err, t1)
		}
		//t.Log(rr)
		//fmt.Printf("after unpack rr is %s.\n", rr.String())
	}

	//fmt.Printf("succceeed.\n")
}
