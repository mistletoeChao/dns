...
func packStructValue(val reflect.Value, msg []byte, off int, compression map[string]int, compress bool) (off1 int, err error) {
	lenmsg := len(msg)
	if val.Type().String() == "dns.XW" {
		if off+8+net.IPv4len > lenmsg {
			return lenmsg, &Error{err: "overflow packing xw"}
		}
		...
		var err error
		off, err = packStructValue(reflect.ValueOf(xw.Hdr), msg, off, compression, compress)
		if err != nil {
			return lenmsg, err
		}

		var weight uint32 = xw.W
		msg[off] = byte(weight >> 24)
		msg[off+1] = byte(weight >> 16)
		msg[off+2] = byte(weight >> 8)
		msg[off+3] = byte(weight)
		off += 4
		var flag uint32 = xw.Flag
		msg[off] = byte(flag >> 24)
		msg[off+1] = byte(flag >> 16)
		msg[off+2] = byte(flag >> 8)
		msg[off+3] = byte(flag)
		off += 4
		if flag == 0 {
			if xw.A != nil && len(xw.A) >= 16 {
				msg[off] = xw.A[12]
				msg[off+1] = xw.A[13]
				msg[off+2] = xw.A[14]
				msg[off+3] = xw.A[15]
			...
			if xw.Target != "" {
				if off, err = PackDomainName(xw.Target, msg, off, compression, true && compress); err != nil {
					return lenmsg, err
				}
			}
		}
		...
		return off, nil
	} else if val.Type().String() == "dns.TRANS" {
		if off+8+net.IPv4len > lenmsg {
			return lenmsg, &Error{err: "overflow packing trans"}
		}
		...
		var err error
		off, err = packStructValue(reflect.ValueOf(trans.Hdr), msg, off, compression, compress)
		if err != nil {
			return lenmsg, err
		}

		var weight uint32 = trans.W
		msg[off] = byte(weight >> 24)
		msg[off+1] = byte(weight >> 16)
		msg[off+2] = byte(weight >> 8)
		msg[off+3] = byte(weight)
		off += 4
		var flag uint32 = trans.Flag
		msg[off] = byte(flag >> 24)
		msg[off+1] = byte(flag >> 16)
		msg[off+2] = byte(flag >> 8)
		msg[off+3] = byte(flag)
		off += 4
		if flag == 0 {
			//fmt.Printf("trans.A:%v len(trans.A):%d\n", trans.A, len(trans.A))
			if trans.A != nil && len(trans.A) >= 16 {
				msg[off] = trans.A[12]
				msg[off+1] = trans.A[13]
				msg[off+2] = trans.A[14]
				msg[off+3] = trans.A[15]
				off += 4
			}
		} else if flag == 1 {
			if trans.Target != "" {
				if off, err = PackDomainName(trans.Target, msg, off, compression, true && compress); err != nil {
					return lenmsg, err
				}
			}
		} else {
			if trans.AAAA != nil && len(trans.AAAA) >= 16 {
				for i := 0; i < 16; i++ {
					msg[off] = trans.AAAA[i]
					off += 1
				}
			}
		}
		...
		return off, nil
	} else if val.Type().String() == "dns.TRANSAA" {
		if off+8+net.IPv4len > lenmsg {
			return lenmsg, &Error{err: "overflow packing transaa"}
		}

		obj := val.Interface()
		transaa, ok := obj.(TRANSAA)
		if !ok {
			return lenmsg, &Error{err: "overflow packing transaa"}
		}

		var err error
		off, err = packStructValue(reflect.ValueOf(transaa.Hdr), msg, off, compression, compress)
		if err != nil {
			return lenmsg, err
		}

		var weight uint32 = transaa.W
		msg[off] = byte(weight >> 24)
		msg[off+1] = byte(weight >> 16)
		msg[off+2] = byte(weight >> 8)
		msg[off+3] = byte(weight)
		off += 4
		var flag uint32 = transaa.Flag
		msg[off] = byte(flag >> 24)
		msg[off+1] = byte(flag >> 16)
		msg[off+2] = byte(flag >> 8)
		msg[off+3] = byte(flag)
		off += 4
		if flag == 0 {
			//fmt.Printf("transaa.A:%v len(trans.A):%d\n", trans.A, len(trans.A))
			if transaa.A != nil && len(transaa.A) >= 16 {
				msg[off] = transaa.A[12]
				msg[off+1] = transaa.A[13]
				msg[off+2] = transaa.A[14]
				msg[off+3] = transaa.A[15]
				off += 4
			}
		} else if flag == 1 {
			if transaa.Target != "" {
				if off, err = PackDomainName(transaa.Target, msg, off, compression, true && compress); err != nil {
					return lenmsg, err
				}
			}
		} else {
			return lenmsg, errors.New("invalid transaa flag")
		}
...
func unpackStructValue(val reflect.Value, msg []byte, off int) (off1 int, err error) {
	...
	if val.Type().String() == "dns.XW" {
		var xw XW
		var err error
		off, err = unpackStructValue(reflect.ValueOf(&xw.Hdr).Elem(), msg, off)
		if err != nil {
			return lenmsg, err
		}

		if xw.Hdr.Rdlength == 0 {
			val.Set(reflect.ValueOf(xw))
			return off, nil
		}
		...
		if off == lenmsg {
			val.Set(reflect.ValueOf(xw))
			return off, nil
		} else if off+int(xw.Hdr.Rdlength) > lenmsg {
			return lenmsg, &Error{"no enough xw unpacking length"}
		} else if off+8 > lenmsg {
			return lenmsg, &Error{"no enough xw unpacking length"}
		}

		xw.W, off = unpackUint32(msg, off)
		xw.Flag, off = unpackUint32(msg, off)
		if xw.Flag == 0 {
			if off+4 <= lenmsg {
				xw.A = net.IPv4(msg[off], msg[off+1], msg[off+2], msg[off+3])
				off += 4
			} else {
				xw.A = net.IP{0, 0, 0, 0}
			}
		} else {
			xw.Target, off, err = UnpackDomainName(msg, off)
			if err != nil {
				return lenmsg, err
			}
		}
		...
		var trans TRANS
		var err error
		off, err = unpackStructValue(reflect.ValueOf(&trans.Hdr).Elem(), msg, off)
		if err != nil {
			return lenmsg, err
		}

		if trans.Hdr.Rdlength == 0 {
			val.Set(reflect.ValueOf(trans))
			return off, nil
		}
		...
		if off == lenmsg {
			val.Set(reflect.ValueOf(trans))
			return off, nil
		} else if off+int(trans.Hdr.Rdlength) > lenmsg {
			return lenmsg, &Error{"no enough trans unpacking length"}
		} else if off+8 > lenmsg {
			return lenmsg, &Error{"no enough trans unpacking length"}
		}

		trans.W, off = unpackUint32(msg, off)
		trans.Flag, off = unpackUint32(msg, off)
		if trans.Flag == 0 {
			if off+4 <= lenmsg {
				trans.A = net.IPv4(msg[off], msg[off+1], msg[off+2], msg[off+3])
				off += 4
			} else {
				trans.A = net.IP{0, 0, 0, 0}
			}
		} else if trans.Flag == 1 {
			trans.Target, off, err = UnpackDomainName(msg, off)
			if err != nil {
				return lenmsg, err
			}
		} else {
			if off+16 <= lenmsg {
				var ipv6Bytes []byte
				for i := 0; i < 16; i++ {
					ipv6Bytes = append(ipv6Bytes, msg[off+i])
				}
				off += 16
				trans.AAAA = net.IP(ipv6Bytes)
			}
		}
		...
		var transaa TRANSAA
		var err error
		off, err = unpackStructValue(reflect.ValueOf(&transaa.Hdr).Elem(), msg, off)
		if err != nil {
			return lenmsg, err
		}

		if transaa.Hdr.Rdlength == 0 {
			val.Set(reflect.ValueOf(transaa))
			return off, nil
		}
		...
		if off == lenmsg {
			val.Set(reflect.ValueOf(transaa))
			return off, nil
		} else if off+int(transaa.Hdr.Rdlength) > lenmsg {
			return lenmsg, &Error{"no enough transaa unpacking length"}
		} else if off+8 > lenmsg {
			return lenmsg, &Error{"no enough transaa unpacking length"}
		}

		transaa.W, off = unpackUint32(msg, off)
		transaa.Flag, off = unpackUint32(msg, off)
		if transaa.Flag == 0 {
			if off+4 <= lenmsg {
				transaa.A = net.IPv4(msg[off], msg[off+1], msg[off+2], msg[off+3])
				off += 4
			} else {
				transaa.A = net.IP{0, 0, 0, 0}
			}
		} else if transaa.Flag == 1 {
			transaa.Target, off, err = UnpackDomainName(msg, off)
			if err != nil {
				return lenmsg, err
			}
		} else {
...
func unpackUint32(msg []byte, off int) (uint32, int) {
	return uint32(msg[off]<<24) | uint32(msg[off+1]<<16) | uint32(msg[off+2]<<8) | uint32(msg[off+3]), off + 4
}
