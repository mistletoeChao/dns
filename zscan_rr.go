...
func setAA(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(AA)
	rr.Hdr = h
	...
	rr.A = net.ParseIP(l.token)
	if rr.A == nil || l.err {
		return nil, &ParseError{f, "bad AA A", l}, ""
	}
	return rr, nil, ""
}

func setAW(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(AW)
	rr.Hdr = h
	...
	weight, err := strconv.Atoi(l.token)
	if err != nil {
		return nil, &ParseError{f, "bad A weight", l}, ""
	}
	rr.W = uint32(weight)
...
func setAAPF(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(AAPF)
	rr.Hdr = h
	...
	rr.AAAA = net.ParseIP(l.token)
	if rr.AAAA == nil || l.err {
		return nil, &ParseError{f, "bad AAPF AAAA", l}, ""
	}
	return rr, nil, ""
}

func setAAAAW(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(AAAAW)
	rr.Hdr = h
	...
	weight, err := strconv.Atoi(l.token)
	if err != nil {
		return nil, &ParseError{f, "bad AAAA weight", l}, ""
	}
	rr.W = uint32(weight)
...
func setCNAMEW(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(CNAMEW)
	rr.Hdr = h
	...
	weight, err := strconv.Atoi(l.token)
	if err != nil {
		return nil, &ParseError{f, "bad CNAME weight", l}, ""
	}
	rr.W = uint32(weight)
...
func setTRANSAA(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(TRANSAA)
	rr.Hdr = h

	l := <-c           // weight
	if l.length == 0 { //Dynamic update
		return rr, nil, ""
	}
	weight, err := strconv.Atoi(l.token)
	if err != nil {
		return nil, &ParseError{f, "bad transaa weight", l}, ""
	}
	rr.W = uint32(weight)
	fmt.Sprintf("transaa weight is %d.\n", rr.W)

	<-c // zBlank
	l = <-c
	//fmt.Printf("transaa prepare body:%s.\n", l.token)
	rr.A = net.ParseIP(l.token)
	if rr.A == nil {
		//fmt.Printf("transaa parse ip failed.\n")
		rr.Flag = 1
		rr.Target = l.token
		if l.token == "@" {
			rr.Target = o
		...
		_, ok := IsDomainName(l.token)
		if !ok || l.length == 0 || l.err {
			return nil, &ParseError{f, "bad transaa Target", l}, ""
		}
		if rr.Target[l.length-1] != '.' {
			rr.Target = appendOrigin(rr.Target, o)
		}
		...
		if rr.A.To4() == nil {
			return nil, &ParseError{f, "bad transaa ipv4 addr", l}, ""
		} else {
			rr.Flag = 0
		}
	}
...
func setTRANS(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(TRANS)
	rr.Hdr = h
	...
	weight, err := strconv.Atoi(l.token)
	if err != nil {
		return nil, &ParseError{f, "bad trans weight", l}, ""
	}
	rr.W = uint32(weight)
	fmt.Sprintf("trans weight is %d.\n", rr.W)

	<-c // zBlank
	l = <-c
	//fmt.Printf("trans prepare body:%s.\n", l.token)
	rr.A = net.ParseIP(l.token)
	if rr.A == nil {
		//fmt.Printf("trans parse ip failed.\n")
		rr.Flag = 1
		rr.Target = l.token
		if l.token == "@" {
			rr.Target = o
		...
		_, ok := IsDomainName(l.token)
		if !ok || l.length == 0 || l.err {
			return nil, &ParseError{f, "bad trans Target", l}, ""
		}
		if rr.Target[l.length-1] != '.' {
			rr.Target = appendOrigin(rr.Target, o)
		}
		//fmt.Printf("trans target: %s.\n", rr.Target)
	} else {
		if strings.IndexByte(l.token, ':') != -1 {
			rr.AAAA = rr.A
			rr.A = nil
			rr.Flag = 2
		} else if rr.A.To4() == nil {
			rr.AAAA = rr.A
			rr.A = nil
			rr.Flag = 2
		} else {
	...
	//fmt.Printf("trans w:%d f:%d A:%v target:%s.\n", rr.W, rr.Flag, rr.A, rr.Target, rr.AAAA)
	return rr, nil, ""
}

func setXW(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(XW)
	rr.Hdr = h
	...
	rr.W = uint32(weight)
	fmt.Sprintf("xw weight is %d.\n", rr.W)

	<-c // zBlank
	l = <-c
	//fmt.Printf("xw prepare body:%s.\n", l.token)
	rr.A = net.ParseIP(l.token)
	if rr.A == nil {
		//fmt.Printf("xw parse ip failed.")
		rr.Flag = 1
		rr.Target = l.token
		if l.token == "@" {
			rr.Target = o
		...
		_, ok := IsDomainName(l.token)
		if !ok || l.length == 0 || l.err {
			return nil, &ParseError{f, "bad XW Target", l}, ""
		}
		if rr.Target[l.length-1] != '.' {
			rr.Target = appendOrigin(rr.Target, o)
		}
		//fmt.Printf("xw target: %s.\n", rr.Target)
	} else {
		rr.Flag = 0
	}

	//fmt.Printf("w:%d f:%d A:%v target:%s.\n", rr.W, rr.Flag, rr.A, rr.Target)
	return rr, nil, ""
}

func setLNAME(h RR_Header, c chan lex, o, f string) (RR, *ParseError, string) {
	rr := new(LNAME)
	rr.Hdr = h
	...
	weight, err := strconv.Atoi(l.token)
	if err != nil {
		return nil, &ParseError{f, "bad LNAME weight", l}, ""
	}
	rr.W = uint32(weight)
	...
	_, ok := IsDomainName(l.token)
	if !ok || l.length == 0 || l.err {
		return nil, &ParseError{f, "bad LNAME Target", l}, ""
	}
	if rr.Target[l.length-1] != '.' {
		rr.Target = appendOrigin(rr.Target, o)
	}
...
var typeToparserFunc = map[uint16]parserFunc{
	...
	TypeCNAMEW:     {setCNAMEW, false},
	TypeXW:         {setXW, false},
	TypeLNAME:      {setLNAME, false},
	TypeTRANS:      {setTRANS, false},
	TypeTRANSAA:    {setTRANSAA, false},
	TypeAA:         {setAA, false},
	TypeAAPF:       {setAAPF, false},
